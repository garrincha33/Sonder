...
step9 = "add touch funtionnailty to add profile picture and store this in firebase - write an extension to add more methods to signup controller makes code more readable, uiimagepickercontrolelrdelegate and uinavigationcontrollerdelegate, add tap gesture to viewdidload profileImage.addGestureRecogniser(), create an object for recognising tap gesture with UITapGestureRecogniser(target and action,   use self and #selector wll be the method implemented by the target), in breif ---- create an extension of signUPViewController implement imagePickercontroller function , create tap gesture add that to profile image and enable user interaction, create function for selector with pickercontroller, pickercontroller delegate = self and present(picker controller) finally - create an if let image = info[UIImagePickerControllerOriginalImage] as? UIImage and assign profile image to image, dissmiss the gallery "

step10 = "store this image on firebase - import firebase storage, create selectedImage VAR at top of project create a reference to FIRStorage location .child(create profile_image).child(user?uid), unwrap with if let profileImage should be value of self.selectedimage (wwhich should be in your extension above profile image, convert profile using JPEG representation 1.0, use storageRef.put, create prfileImageURL = meta?downloadURL()?.absoultestring, move database stuff to your closure and append your url string to your newwuserReference dictionary "
